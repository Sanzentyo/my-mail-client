use rust_thread_messenger::command::CommandType;
                    ...
                    match &recv_json.command {
                        CommandType::SendMsg => {
                            match recv_json.args {
                                ...
                                _ => {
                                    let invaild_json = SearchMsgResponse {
                                        status: ResponseStatus::Invalid,
                                        timestamp: Utc::now().timestamp(),
                                        msg: Vec::new(),
                                    };
                                    ...
                                    writer.write_all(json.as_bytes()).await.unwrap();
                                    writer.flush().await.unwrap();
                                    println!("Invalid command(SendMsg)");
                                },
                        }
                        },
                        CommandType::ListMsg => {
                            match recv_json.args {
                                ...
                                _ => {
                                    ...
                                    writer.write_all(json.as_bytes()).await.unwrap();
                                    writer.flush().await.unwrap();
                                    println!("Invalid command(ListMsg)");
                                },
                            }
                        },
                        CommandType::SearchMsg => {
                            match recv_json.args {
                                Args::SearchMsg(args) => {
                                ...
                                _ => {
                                    ...
                                    writer.write_all(json.as_bytes()).await.unwrap();
                                    writer.flush().await.unwrap();
                                    println!("Invalid command(SearchMsg)");
                                },
                            }
                        }
                    }
